...
namespace MerchantTribe.Commerce
{
    public class CacheManager
    {
        ...
        // Category Snapshots
        public static void SetCategorySnaps(List<Catalog.CategorySnapshot> snaps, long storeId)
        {
            StoreItem<List<Catalog.CategorySnapshot>>("categorysnapshots" + storeId, snaps, 60);
        }
        public static List<Catalog.CategorySnapshot> GetCategorySnaps(long storeId)
        {
            var cache = Current();
            if (cache != null)
            {
                var i = cache["categorysnapshots" + storeId];
                if (i != null)
                {
                    return (List<Catalog.CategorySnapshot>)i;
                }
            }
            return null;
        }
        public static void ClearCategorySnaps(long storeId)
        {
            ClearItem("categorysnapshots" + storeId);
        }
